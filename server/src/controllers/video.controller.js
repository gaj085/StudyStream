const { createJob, getJob, updateJobProgress, completeJob, failJob } = require('../services/job.service');
...
// In-memory store for metadata (to avoid DB overhead for this MVP)
const videoStore = {};
...
async function processVideoBackground(jobId, youtubeUrl) {
  try {
    ...
    updateJobProgress(jobId, 'transcribing', 20, 'Fetching transcript from YouTube...');
    let segments;
    try {
      segments = await fetchTranscript(youtubeUrl);
    } catch (err) {
      throw new Error('No usable YouTube captions were found for this video. Please try a video with subtitles enabled.');
    }
    ...
    // 2. Chunking
    updateJobProgress(jobId, 'chunking', 40, 'Chunking transcript with timestamp preservation...');
    const chunks = chunkTranscript(segments);

    // 3. Embedding
    updateJobProgress(jobId, 'embedding', 60, 'Generating local embeddings via MiniLM...');
    ...
    updateJobProgress(jobId, 'indexing', 80, 'Indexing embeddings in ChromaDB...');
    await deleteChunks(videoId);
    await storeChunks(videoId, chunks, embeddings);

    // 5. Generate Summary and Quiz
    updateJobProgress(jobId, 'summarizing', 90, 'Generating summary, key concepts, and quiz...');
    let summary, quiz;
    try {
      summary = await generateSummary(segments);
    } catch (err) {
      throw new Error('Learning content could not be generated. Please try again.');
    }
    try {
...
async function startVideoProcessing(req, res) {
  ...
  if (!url.includes('youtube.com') && !url.includes('youtu.be')) {
    return res.status(400).json({ error: 'Invalid YouTube URL. Please paste a valid YouTube link.' });
  }
...
async function getSummary(req, res) {
  ...
  if (!video || !video.summary) {
    return res.status(404).json({ error: 'Summary not found. The video may still be processing.' });
  }
...
async function getQuiz(req, res) {
  ...
  if (!video || !video.quiz) {
    return res.status(404).json({ error: 'Quiz not found. The video may still be processing.' });
  }
