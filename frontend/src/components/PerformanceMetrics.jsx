...
function PerformanceMetrics({ latency }) {
  ...
  const metrics = [
    ...
    { label: 'Retrieval', value: latency.retrieval, color: 'text-purple-400' },
    { label: 'LLM', value: latency.generation, color: 'text-cyan-400' },
    { label: 'Total', value: latency.total, color: 'text-emerald-400', bold: true },
  ];
    ...
    <div className="border-t border-slate-800/50 pt-3 mt-3">
      ...
      {expanded && (
        <div className="mt-3 grid grid-cols-2 gap-2">
          {metrics.map((m) => (
            <div key={m.label} className="flex justify-between items-center px-3 py-2 bg-slate-950/60 rounded-lg border border-slate-800/50">
              <span className="text-[11px] text-slate-500">{m.label}</span>
              <span className={`text-[11px] font-mono font-semibold ${m.color} ${m.bold ? 'text-xs' : ''}`}>
                {formatMs(m.value)}
              </span>
            </div>
          ))}
